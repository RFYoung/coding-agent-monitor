...
| Runtime validation is surface-aware and not browser-only | Proven for control logic | `validation_surface` evidence-tier tests; runtime validation docs; web/mobile/native/system/ML entropy tests; auto-discovered project markers; `config verifier` can register explicit runtime mappings | Real platform validation still requires an observed or configured platform verifier before a runtime probe can succeed. Weak path/text hints cannot force the obligation alone. |
